...
def cache_builder(edges, comment_size, force_cache, loc_add=0, loc_del=0):
    ...
    # 确保 cache 目录存在，以防在 Actions 环境中不存在
    ...
    try:
        ...
        lines = []
        if comment_size > 0:
            for _ in range(comment_size): lines.append('This line is a comment block. Write whatever you want here.\n')
        with open(filename, 'w') as f:
            f.writelines(lines)

    if len(lines)-comment_size != len(edges) or force_cache: # If the number of repos has changed, or force_cache is True
        cached = False
    ...
    lines = lines[comment_size:] # remove those lines
    for index in range(len(edges)):
        repo_hash, commit_count, *__ = lines[index].split()
        if repo_hash == hashlib.sha256(edges[index]['node']['nameWithOwner'].encode('utf-8')).hexdigest():
            try:
                if int(commit_count) != edges[index]['node']['defaultBranchRef']['target']['history']['totalCount']:
                    # if commit count has changed, update loc for that repo
                    owner, repo_name = edges[index]['node']['nameWithOwner'].split('/')
                    loc = recursive_loc(owner, repo_name, lines, cache_comment)
                    lines[index] = repo_hash + ' ' + str(edges[index]['node']['defaultBranchRef']['target']['history']['totalCount']) + ' ' + str(loc[2]) + ' ' + str(loc[0]) + ' ' + str(loc[1]) + '\n'
            except TypeError: # If the repo is empty
                lines[index] = repo_hash + ' 0 0 0 0\n'
    with open(filename, 'w') as f:
